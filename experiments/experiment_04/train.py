...
def main():
    ...
    print(f"Experiment 04: Hybrid SlowFast on {DEVICE}")

    mlflow.set_experiment("SlowFast_Anticipation")

    with mlflow.start_run(run_name="Experiment_04_Hybrid"):
        mlflow.log_params({"model": "Hybrid_SlowFast", "lr": LR})

        if not os.path.exists(DATA_PATH):
            pass

        model = HybridSlowFast(num_classes=400).to(DEVICE)

        criterion = ActionAnticipationLoss(alpha=0.5)  # Hybrid uses loss penalty
        ...
        try:
            ...
            for epoch in range(EPOCHS):
                ...
                if val_acc1 > best_acc:
                    ...
                    mlflow.log_artifact("best_hybrid.pth")
