...
def main():
    ...
    print(f"Experiment 03: ROI SlowFast on {DEVICE}")

    mlflow.set_experiment("SlowFast_Anticipation")

    with mlflow.start_run(run_name="Experiment_03_ROI"):
        mlflow.log_params({"model": "ROI_SlowFast", "lr": LR})

        if not os.path.exists(DATA_PATH):
            pass

        model = RoiGuidanceSlowFast(num_classes=400).to(DEVICE)
        ...
        try:
            ...
            for epoch in range(EPOCHS):
                ...
                if val_acc1 > best_acc:
                    best_acc = val_acc1
                    torch.save(model.state_dict(), "best_roi.pth")
                    mlflow.log_artifact("best_roi.pth")
