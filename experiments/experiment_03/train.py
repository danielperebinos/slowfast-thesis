"""
Experiment 03 — ROI-Guided SlowFast R50 (YOLOv8 + Hadamard).

YOLOv8 detections on the Fast pathway are converted to binary ROI masks and
applied via a Hadamard product (alpha_blend=1.0 → full masking, 0.0 → no-op).
The loss adds a configurable temporal-proximity penalty (LOSS_ALPHA sweep).
...
def main():
    ...
    print(f"Experiment 03: ROI-Guided SlowFast R50 | device={DEVICE}")
    ...
    with mlflow.start_run(run_name="Experiment_03_ROI"):
        mlflow.log_params(
            {
                "model":       "ROI_SlowFast_R50",
                "num_classes": num_classes,
                "epochs":      EPOCHS,
                "batch_size":  BATCH_SIZE,
                "lr":          LR,
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            if val_acc > best_val_acc:
                ...
                best_path = os.path.join(EXPERIMENT_DIR, "best_roi.pth")
